...
interface PropsCamera extends CameraViewProps {
  ...
  onBarCodeScanned(
    scanningResult: PropsBarCodeScanningResult
  ): Promise<void> | void;
}

export const regexVolumeEntrega =
  /\/etiqueta\/expedicao\/|[0-9]+_[0-9]+_[0-9]+$/g;
export const regexQRCliente =
  /(^C[0-9]+$|[A-z0-9-]{36}_[0-9]+_(TROCA|ENTREGA))/g;
export const regexQRPainel = /^P[0-9]+$/g;
export const regexSkuLegado = /^SKU_[0-9]+_[0-9]+$/g;
export const regexSku = /^SKU[0-9]+$/g;

type TypeEstadoDaCamera =
  | "SEM_PERMISSAO"
  | "DISPONIVEL"
  | "INDISPONIVEL"
  | "CAMERA_PAUSADA";

export function verificaLeituraScanner(
  dado: string,
  type: string
): PropsLeitura {
  ...
  switch (true) {
    ...
    case type === "qr" && dado?.match(/produto\/+[0-9]+[?w=]/g)?.length === 1:
      ...
      break;
    case type === "qr" &&
      (dado?.length < 5 || !!dado?.match(regexQRPainel)?.length):
...
export function Camera(props: PropsCamera) {
  ...
  async function beforeOnBarCodeScanned(
    ...
    const tipoDeCodigoLido = verificaLeituraScanner(
      scanningResult.data,
      scanningResult.type
    );

    if (
      (props.ignorarLeitura?.length &&
        props.ignorarLeitura?.includes(tipoDeCodigoLido)) ||
      (props.aceitaLeitura?.length &&
        !props.aceitaLeitura?.includes(tipoDeCodigoLido))
    ) {
      return;
    }
  ...
  return (
    <Container>
      ...
      {estadoDaCamera === "CAMERA_PAUSADA" && (
        <CameraPausada reativarCameraInativa={reativarCameraInativa} />
      )}
      <Conteudo>
        {typeof props.cabecalho === "string" ? (
          <CardInfo texto={props.cabecalho} />
        ) : (
          props.cabecalho
        )}
        {estadoDaCamera !== "CAMERA_PAUSADA" && (
          <BotaoAtivador
            ativacaoManual={!!props.ativacaoManual}
            quandoTocar={ativarCamera}
          >
            <AlvoContainer>
              {cameraProcessando && (
