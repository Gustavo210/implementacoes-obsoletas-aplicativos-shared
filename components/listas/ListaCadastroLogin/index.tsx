...
import { UsuarioSelecionadoLogin } from '@/hooks/(legacy)/useAuth'

import { ListaCadastrosLoginItem } from '../ListaCadastroLoginItem'

type ListaCadastrosProps = {
  usuarios: UsuarioSelecionadoLogin[]
  selecionarUsuario(id_colaborador: number, telefone: string, razao_social: string, foto_perfil: string): void
}

function getItem(data: UsuarioSelecionadoLogin[], index: number): UsuarioSelecionadoLogin {
  return data[index]
}

export function ListaCadastrosLogin(props: ListaCadastrosProps) {
  function escolherUsuario(idColaborador: number, telefone: string, razaoSocial: string, fotoPerfil: string) {
  ...
  return (
    <VirtualizedList
      ...
      renderItem={({ item }) => <ListaCadastrosLoginItem selecionarUsuario={escolherUsuario} {...item} />}
