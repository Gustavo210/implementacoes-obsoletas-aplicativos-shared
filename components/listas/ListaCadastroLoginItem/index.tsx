...
type ListaCadastroItemProps = UsuarioSelecionadoLogin & {
  selecionarUsuario(id_colaborador: number, telefone: string, razao_social: string, fotoUsuario: string): void
}

export function ListaCadastrosLoginItem(props: ListaCadastroItemProps) {
  function escolherUsuario(id: number, telefone: string, razaoSocial: string, fotoUsuario: string) {
  ...
  return (
    <BlocoCadastro
      onPress={() =>
        escolherUsuario(Number(props.id_colaborador), props.telefone, props.razao_social, props.foto_perfil)
      }
    >
      <ContainerCadastro>
